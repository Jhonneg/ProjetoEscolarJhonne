...
function cadastrar() {
  ...
  const loginJaExiste = usuarios.some(function (u) {
    return u.login.toLowerCase() === login.toLowerCase();
  });
...
function salvarCurso() {
  ...
  const jaExiste = cursos.some(function (c) {
...
function renderizarCursos() {
  ...
  if (cursos.length === 0) {
    lista.innerHTML =
      '<p class="lista-vazia">Nenhum curso cadastrado ainda.</p>';
    return;
  }

  lista.innerHTML = cursos
    .map(function (curso) {
      return (
        '<div class="list-item">' +
        '<div class="list-avatar av-curso">' +
        curso.sigla +
        "</div>" +
        '<div class="list-info">' +
        '<div class="list-nome">' +
...
function salvarAluno() {
  ...
  const emailJaExiste = alunos.some(function (a) {
  ...
  const cursos = JSON.parse(localStorage.getItem(CHAVE_CURSOS) || "[]");
  const cursoObj = cursos.find(function (c) {
    return String(c.id) === String(cursoId);
  });
...
function renderizarAlunos() {
  ...
  if (alunos.length === 0) {
    lista.innerHTML =
      '<p class="lista-vazia">Nenhum aluno cadastrado ainda.</p>';
    return;
  }
  ...
  lista.innerHTML = alunos
    .map(function (aluno) {
      ...
      return (
        '<div class="list-item">' +
        '<div class="list-avatar av-aluno">' +
        iniciais +
        "</div>" +
        '<div class="list-info">' +
        '<div class="list-nome">' +
